...
public class Weapon : MonoBehaviour
{
	...
	public int crosshairLength = 20;					// The length of each crosshair line
	public int crosshairWidth = 8;						// The width of each crosshair line
	...
	public void CheckForUserInput()
	{
		...
		if (type == WeaponType.Beam)
		{
			if (Input.GetMouseButton(0) && beamHeat <= maxBeamHeat && !coolingDown)
			{
				Beam();
			}
			else
	...
	void OnGUI()
	{
        ...
        centerStyle.fontSize = 90;
        centerStyle.normal.textColor = new Color(255f / 256f, 164f / 256f, 0f / 256f, 255f / 256f);
		...
		if (showCurrentAmmo)
		{
			if (type == WeaponType.Raycast || type == WeaponType.Projectile)
                GUI.Label(new Rect(20, Screen.height - 100, 100, 20), "Ammo: " + currentAmmo, guiStyle);
			else if (type == WeaponType.Beam)
                GUI.Label(new Rect(20, Screen.height - 100, 100, 20), "Heat: " + (int)(beamHeat * 100) + "/" + (int)(maxBeamHeat * 100), guiStyle);
		}
        ...
        if (true)
        {
            ...
            guiStyle.normal.textColor = new Color(255f / 256f, 164f / 256f, 0f / 256f, 255f / 256f);
            GUI.Label(new Rect(270, 70, 100, 20), "× " + Health.DESTROY_NUM, guiStyle);
            GUI.Label(new Rect(270, 320, 100, 20), "× " + (Spawner.count - Health.DESTROY_NUM), guiStyle);
            //GUI.Label(new Rect(270, 600, 100, 20), Screen.height.ToString(), guiStyle);
            //GUI.Label(new Rect(270, 700, 100, 20), Screen.width.ToString(), guiStyle);
            GUI.Label(new Rect(20, Screen.height - 200, 100, 20), "Lev. " + Health.LEVEL, guiStyle);
            if ((Spawner.count - Health.DESTROY_NUM) >= 5)
            {
                //提示游戏失败
                Time.timeScale = 0;
                GUI.Label(new Rect(Screen.width / 2 - Screen.width / 4, Screen.height / 3, Screen.width / 2, 1), "Mission Failed!", centerStyle);
                if (GUI.Button(new Rect(Screen.width / 2 - Screen.width / 8, Screen.height / 3 * 2 - Screen.height / 16, Screen.width / 4, Screen.height / 8), "Play again"))
                {
                    //重新加载场景  
                    Health.DESTROY_NUM = 0;
                    Health.LEVEL = 0;
                    Spawner.count = 1;
                    Spawner.freqLevel = 0;
                    Spawner.spawnFrequency = 6.0f;
                    SceneManager.LoadScene("ARMonsters");
                    Time.timeScale = 1;
                }
            }
            else if (Health.DESTROY_NUM >= 20)
            {
                //游戏胜利
                Time.timeScale = 0;                
                GUI.Label(new Rect(Screen.width / 2 - Screen.width / 4, Screen.height / 3, Screen.width / 2, 1), "Mission Accomplished!", centerStyle);
                if (GUI.Button(new Rect(Screen.width / 2 - Screen.width / 8, Screen.height / 3 * 2 - Screen.height / 16, Screen.width / 4, Screen.height / 8), "Play again"))
                {
                    //重新加载场景  
                    Health.DESTROY_NUM = 0;
